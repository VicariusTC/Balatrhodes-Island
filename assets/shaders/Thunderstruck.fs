...
float lightning(vec2 uv, float t, float intensity) {
    float cycle = fract(t);
    float collapse = smoothstep(0.15, 0.55, cycle);

    float x = 0.5 
            + 0.10 * sin(uv.y * 7.0 + intensity) 
            + 0.05 * sin(uv.y * 17.0 - intensity) 
            + 0.03 * sin(uv.y * 37.0);

    x += (noise(vec2(uv.y * 8.0 + intensity, floor(t * 8.0))) - 0.5) * 0.12;

    float width = mix(0.28, 0.05, collapse);
    float d = abs(uv.x - x);
    float strike = 1.0 - smoothstep(width, width + 0.06, d);

    return clamp(strike, 0.0, 1.0);
}
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    float bolt_time = thunder.y * 0.8;
    float bolt_intensity = thunder.x * 0.5; 

    float bolt = lightning(uv, bolt_time, bolt_intensity);
    float glow = lightning(uv + vec2(0.01, 0.0), bolt_time, bolt_intensity);

    vec3 lightning_color = vec3(1.0, 1.0, 1.0);
    
    tex.rgb = mix(tex.rgb, lightning_color, bolt);
    tex.rgb += glow * (0.15 + (bolt_intensity * 0.1));
...
//largely just 1:1 from the Steamodded example I am never doing this again